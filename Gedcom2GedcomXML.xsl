...
***************************************************************************
**                                                                       **
**    Templates Related to the Header and its associated element         **                                                 
**        * HEAD -> <HeaderRec>                                          **
**        * SOUR -> <FileCreation>                                       **
**        * DATA -> <Caption>                                            **
**                                                                       **
...
***************************************************************************
************************************************************************ -->
<!-- Some of the mapping here are suspect because the SOUR tags purpose
   is ambiguous -->
...
<xsl:template match="SOUR" mode="HeaderRec">
     <FileCreation>
        ...
        <!-- TODO the TIME attribute is optional but it can be implement on a contingent basis-->
...
***************************************************************************
**                                                                       **
**    Templates related to the IndividualRecs and its associated         **
**    elements                                                           **                                                 
**        * INDI -> <IndividualRec>                                      **
**        * NAME -> <IndivName>                                          **
...
<!-- **********************************************************************
    ...
    NAME Template which returns the Surname - for use when INDI is a
...
<!-- **********************************************************************
    ...
    EventRecs Template - calls templates that create individual, family
    and LDS EventRecs

************************************************************************ -->
<xsl:template name="EventRecs">
     <!-- INDIVIDUAL_EVENT_STRUCTURE -->
     <!-- Both BIRT and ADOP are handled separately because  
          GEDCOM 5.5 allows to add other participants 
...
<!-- **********************************************************************
    ...
    CAUS Template - GEDCOM 6.0 XML eliminates this tag, but it is important
        information.  To include with a individual it is surrounded by a
        Note element 
...
<xsl:template name="addEventPlace">
    <xsl:choose>
        ...
        The reason I choose this implementation is because I wanted to preserve 
        the Township and County data.  Perhaps another implementation would utilize the FORM tag
        -->
...
***************************************************************************
    ...
    ADDR Template mode=Place - Handles ADDR which occur in INDI, OBJE and
        ...
        inside the GroupRec and the ContactRec and these cannot be linked to
...
<!-- DOC Again this isn't the most ideal solution for handling this GEDCOM 5.5 structure:
  ...
  building name, business name, or personal name. It then places this data inside
...
<xsl:template match="ADDR" mode="Place">
 ...
 <!-- **********************************************************************
    ...
    CONT Template mode=Place - handles the CONT tag that may occur
        ...
        of a Place element it simply pads each ADDR's CONT line with a 
        whitespace
     ...
     <EventRec>
        ...
        <xsl:if test="$tag = 'CREM'">
         ...
         <Participant>
             ...
             <Role>principal</Role>
             <!-- OP I refuse to implement the Living element because it is
             	 pointless for the most part  and the spec only implies that it
             	 is valid only for ordination; that is, only for
...
************************************************************************ -->
<!-- DOC? 
     Events in GEDCOM 6.0 seems to be ontologically different from events
     in 5.5.  The difference is indicated by the <Participant> tag. 
     Theorectially, the <Participant> tag could mark, say, all the participant
     at funeral/burial; i.e, the deceased and the ballbearers, etc.  This
     has implications, I believe, for the BIRT tag translation into GEDCOM 6.0.
     Events are more strongly associated at a particular time.  If someone
     wasn't at the event, they are not a participant. In the case of a BirthEvent
     the only sure participant, as far as the BIRT tag indicates, is the 
     mother and the child.  The father could be dead or may not be there at
     the hospital at the time.  If he is indeed the biological father of the
     child, he was, of course, at the event of conception, but there is
     a definite possibly that he was not present at the event of birth. Hence,
     the only guaranteed participant at this event would be the mother. This
     implementation always associated the birth event with the mother and not
     the father. 
     
    One drawback with this approach is that a birth of a father's
    child does not show up in the record of his life events.  
 -->
...
<!-- **********************************************************************
    ...
    ADOP Template - this templates creates an ADOP EventRect and attempts
        to add the adoptive parents at the event by calling the FAMC template
...
<!-- **********************************************************************
    ...
    FAMC Template mode=AdoptionEvent - when called from the ADOP template
        ...
        to this event.  To do so, it tests the value of the ADOP tag whose
 ...
 <!-- Added mode because of the CENS tag which can also be a Family Event -->
     <xsl:variable name="tag" select="name()"/>
       <EventRec>
        ...
        <!-- Handles All FAM events that one would need to mention the involvment of children. 
        		Of course, there could be children preceding the marriage but this situation 
        		is difficult to determine using GEDCOM 5.5 structures 
        		(Theorectically, this may be accounted for if one compares the date
...
<xsl:template match="EVEN" mode="Family">
     <EventRec>
        ...
        <!-- DOC Assumes that all EVEN that occur in a FAM apply to all children of the FAM -->
        <xsl:apply-templates select="../CHIL" mode="Events"/>
...
<!-- **********************************************************************
    ...
    addEventEvidence Template - adds the PLAC sources as Evidence
        to the EventRec
...
***************************************************************************
    ...
    To handle this tag, the template creates Evidence and its associated
        elements

***************************************************************************
************************************************************************ -->
<!-- DOC should note where that the SOURCE_DESCRIPTION has been mapped
    to Caption Element -->
...
***************************************************************************        
************************************************************************ -->
<!-- DOC Current implementation discards the valid OBJE or OBJE @O1@ tag inside the SOUR. -->
         ...
         <Citation>
            ...
            <xsl:if test="DATA/DATE">
             ...
             <!-- Since we do not want to loose the TEXT from the 0 @S*@ SOUR, because it
                 ...
                 is now imported into the Evidence element -->
...
<xsl:template match="CORP">
    <ContactRec>
        ...
        <!-- This call assumes that the ADDR structure under the CORP tag
        	    actually repeats the business' name. Since the ADDR structure 
        	    is one level deeper it would seem to suggest that the ADDR text contains
            a street address in fact.  A with-param could be passed to this
            template, if it is implemented, but there is no way to tell what
            is in the CONT tag.
        -->
...
<xsl:template match="SUBM[@ID]">
    <ContactRec>
        ...
        <!-- TODO?  implement Type attribute with the following values:
        ...
        <!-- Handles OBJE hits as either a <Evidence> Element or as a SourceRec link -->
        <xsl:apply-templates select="OBJE"/>
...
<xsl:template match="ADDR" mode="MailAddress">
     ...
     <!-- Strange, if text() is passed as the default of PlaceName it
        return the text of the children and this text() doesn't -->
...
*********************************************************************** -->
<!-- FIX if at all possible.  Usually the PHON tag belongs 
    to the MailAddress ContactRec The problem is that GEDCOM 5.5 
    places the PHON and the ADDR tags at the same
    level.  The current implementation simply surrounds the Phone
    element with at ContactRec, losing, IOW,
...
***************************************************************************
    ...
    FAM Template - creates the FamilyRec and related elements, checking
        to determine if there is a MARR event supporting this, checking
...
<!-- **********************************************************************
    ...
    NOTE[@REF] Template - handles a linked NOTE_STRUCTURE or a 
        ...
        that.  Their contents willinstead be placed in the element which
        referenced them.
...
***************************************************************************
    ...
    NOTE[@ID] Template - handles a linked NOTE_STRUCTURE or a 
        reference to notes.  NOTE tags that reference a NOTE_RECORD will
        not be linked in GEDCOM XML 6.0 because there is no mechanism for
        that.  Their contents will instead be placed in the element which
        referenced them.
...
<!-- **********************************************************************
    ...
    ExternalIDs template - called to add ExternalId elements

*********************************************************************** -->
<!-- DOC at one place in the 6.0 draft specification it says there are only 2 values for attribute 
...
<!-- **********************************************************************
    ...
    NOTE and NOTE[@REF] templates mode Changed- helps templates
...
<!-- **********************************************************************
    ...
    Submitter Template creates the Submitter elements and links it
        ...
        GEDCOM 5.5 which allows for multiple SUBM.  Gedcom 6.0 XML
        only allows for one.  Consequently, this implementation is lossy.
...
<xsl:template match="STAT">
    ...
    <OrdStat>
        <xsl:attribute name="Code">
            ...
            <!-- There is no GEDCOM 6.0 equivalent to INFANT and PRE-1970, 
                 they are mapped to 'completed'-->
            <xsl:if test="($status != 'INFANT') and ($status != 'PRE-1970' )">
